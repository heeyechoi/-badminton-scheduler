...
- 급수 → 이름(가나다) 순 정렬, 성별 필터(전체/남자/여자), 급수 칩 필터, "게임중인 사람 가리기", "게임 수 적은 순"
- **게임 수 표시/정렬은 항상 `effectiveGameCount()`(완료+진행중+예약 합산) 하나만 써야 함** — 예전엔 정렬이 원본 `totalGames`(완료분만)를 써서 "7게임 표시된 사람이 6게임 표시된 사람보다 먼저 뜨는" 버그가 있었음(수정 완료)
- 참가자 카드: 게임 수, 성별/급수 타입별(혼복·남복·여복) 누적 횟수, 혼복 대비 동성복식 횟수 차이가 2 이상이면 주황 강조. 모바일(≤640px)에서만 게임 수가 이름·급수 옆으로 이동, PC/태블릿은 기존 위치(하단) 유지
- 헤더 한 줄 구성: "참가자ⓘ | 전체·남자·여자 | 게임중인사람가리기·게임수적은순 토글 | 햄버거메뉴(☰: 참가자관리/엑셀추가/참가자추가)"
