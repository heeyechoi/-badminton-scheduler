...
- **"매칭가능한 선수만" 켜면 대기열에 이미 예약된(`reservedIds`) 선수만 흐리게(dim) 처리** — 게임중이거나 휴식중이어도 아직 대기열에 예약 안 됐다면 새 대기열 게임에 넣을 수 있으므로(즉시 시작은 안 되고 대기열로 감) 원래 색상 유지. 진짜로 다시 못 뽑는 경우는 "이미 다른 대기열 게임에 커밋됨" 하나뿐. DOM에서 제거하지 않고 dim만 하는 원칙은 유지(레이아웃 점프 방지)
