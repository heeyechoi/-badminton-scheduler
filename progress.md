...
- ⚠️ **알려진 불일치**: "휴식중은 대기열에도 못 넣는다"는 규칙이 카드 흐림 표시에는 반영됐지만, 실제 수동매칭/추천매칭에서 휴식중인 선수를 선택해 대기열에 등록하는 것 자체는 아직 막혀있지 않음(`ParticipantCard`의 `selectable`은 `!reserved`만 체크, `_createGameFromPlayerIds`도 게임중/휴식중 멤버가 있으면 즉시 코트 배정만 안 할 뿐 대기열 등록은 허용) — 사용자 확인 후 이 부분도 막을지 결정 필요
