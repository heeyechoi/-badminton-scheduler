...
- **정렬 기준을 급수순/이름순 중에 고를 수 있음**(`filters.sortBy: 'skill' | 'name'`, 기본값 `'skill'`): "급수순"은 기존 급수→이름(가나다) 정렬, "이름순"은 급수 무시하고 순수 가나다순 — `selectors.js`의 `baseSortComparator`가 이 둘을 만들고, `filteredParticipants`는 그 위에 기존 "매칭가능한 선수 그룹이 위로" 재정렬과 "게임 수 적은 순" 재정렬을 그대로 얹음(둘 다 stable sort라서 동점일 때 급수순/이름순 선택이 타이브레이커로 자연히 살아남음). **"게임 수 적은 순"이 꺼져 있으면 게임 수는 정렬에 전혀 관여하지 않음**(총 게임 수가 제각각인 참가자들로 직접 확인함) — 다만 "매칭가능한 선수 그룹이 위로" 오는 기존 규칙(게임 수가 아니라 현재 게임중/예약 여부 기준)은 급수순/이름순 여부와 무관하게 그대로 살아있음. 급수순/이름순 칩 + 참가자 관리 햄버거 메뉴는 `panel-header-actions` 안에서 `panel-header-sort-menu`로 따로 묶어 `margin-left:auto`로 항상 우측 끝(패널 헤더 줄의 오른쪽)에 붙게 함 — "매칭가능한 선수만"/"게임 수 적은 순" 토글은 `panel-header-toggles`로 별도 묶어서, 좁은 화면에서 줄바꿈이 일어나도 정렬/메뉴 그룹이 딴 데로 안 밀리게 함. 성별 필터(전체/남자/여자), 급수 칩 필터, "매칭가능한 선수만"(구 "게임중인 사람 가리기"), "게임 수 적은 순"
...
- **비동호인 카드 색상**: 고정 팔레트(자강처럼)가 아니라 **현재 설정된 남/여 색상에서 한 톤 어둡게** — `tokens.css`의 `--color-male-nonmember`/`--color-female-nonmember`(+`-bg` 변형)가 `color-mix(in srgb, var(--color-male) 65%, #334155)` 식으로 어두운 슬레이트색과 섞어서 계산함(설정에서 남/여 색상을 커스텀해도 항상 그 색 기준으로 유지됨). **주의: `hsl(from var(--color-male) h calc(s*0.55) calc(l-15%))` 같은 relative-color 문법으로 먼저 시도했다가 실패함** — 이 브라우저 환경에서 `h`/`s` 채널에 대한 `calc()`는 되는데 `l`(lightness) 채널에 `calc()`를 쓰면 계산값이 invalid 처리되어 배경이 통째로 투명해짐(커스텀 프로퍼티가 invalid하면 해당 값을 쓰는 선언 자체가 초기값으로 폴백됨) — 그래서 `color-mix()`만으로 재구현함. 처음엔 `color-mix(..., black)`(순수 검정과 혼합)로 했다가 "너무 어둡고 탁하다"는 피드백을 받아 어두운 슬레이트(`#334155`, 채도가 낮은 색)와 섞는 방식으로 바꿈 — 명도만 낮추는 것과 채도까지 같이 낮추는 것의 체감 차이가 커서, 색 어둡게 처리가 필요할 때는 검정이 아니라 저채도 중간톤과 섞는 걸 기본으로 고려할 것. `genderColorClass()`가 비동호인이면 `is-nonmember` 클래스를 붙이고, 자강과 동일한 패턴으로 관리자 쪽 9개 컴포넌트 CSS에 다크 변형을 미러링함(미리보기 2개는 위 항목대로 의도적으로 제외). 참가자 카드의 "비동호인" 배지는 4글자라 다른 급수 배지보다 폰트 크기를 살짝 줄임(`ParticipantCard.css`).
