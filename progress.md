...
- 참가자 카드: 게임 수, 성별/급수 타입별(혼복·남복·여복) 누적 횟수, 혼복 대비 동성복식 횟수 차이가 2 이상이면 주황 강조. 모바일(≤640px)에서만 게임 수가 이름·급수 옆으로 이동, PC/태블릿은 기존 위치(하단) 유지. 이름이 길어서(외국인 이름 등) 2줄로 줄바꿈되는 카드는 `.participant-card-main`을 `align-items: flex-start`로, 급수 배지엔 `white-space: nowrap` + `flex-shrink: 0`을 줘서 배지가 이름 첫 줄에 맞춰 한 줄로만 뜨게 함(예전엔 `align-items: center`라 배지가 2줄 이름의 세로 중앙으로 밀리고, 배지 자체 텍스트까지 줄바꿈되던 문제가 있었음)
...
- 헤더 2줄 구성: 1줄 "참가자ⓘ | 전체·남자·여자 ......... 급수순·이름순 | 햄버거메뉴(☰: 참가자관리/엑셀추가/참가자추가)", 2줄 "매칭가능한선수만·게임수적은순 토글"(위 정렬 항목 참고)
...
- **비동호인은 관리자 화면에만 보임**: `src/data/skillLevels.js`의 `participantFacingSkill(skill)`이 비동호인일 때 `null`을 반환 — 미리보기/참여자 화면(`DisplayCourtCard`, `DisplayQueueItem`)은 이 함수를 거쳐서 급수 라벨 자체를 안 보여줌(게임 수만 표시), 카드 색상도 그 두 파일엔 `is-nonmember` CSS 규칙을 아예 안 넣어서 일반 성별 색상 그대로 보임. 관리자 쪽 컴포넌트(참가자 카드/코트/대기열/수동매칭/추천매칭/게임로그)는 급수를 그대로 넘기므로 배지가 정상 표시됨(단, 화면에 찍히는 텍스트는 아래 "비동호인 표시 텍스트" 항목대로 "입문"으로 바뀌어 있음 — 참여자 화면에선 그마저도 안 보임).
- **비동호인 카드 색상**: 고정 팔레트(자강처럼)가 아니라 **현재 설정된 남/여 색상에서 한 톤 어둡게** — `tokens.css`의 `--color-male-nonmember`/`--color-female-nonmember`(+`-bg` 변형)가 `color-mix(in srgb, var(--color-male) 65%, #334155)` 식으로 어두운 슬레이트색과 섞어서 계산함(설정에서 남/여 색상을 커스텀해도 항상 그 색 기준으로 유지됨). **주의: `hsl(from var(--color-male) h calc(s*0.55) calc(l-15%))` 같은 relative-color 문법으로 먼저 시도했다가 실패함** — 이 브라우저 환경에서 `h`/`s` 채널에 대한 `calc()`는 되는데 `l`(lightness) 채널에 `calc()`를 쓰면 계산값이 invalid 처리되어 배경이 통째로 투명해짐(커스텀 프로퍼티가 invalid하면 해당 값을 쓰는 선언 자체가 초기값으로 폴백됨) — 그래서 `color-mix()`만으로 재구현함. 처음엔 `color-mix(..., black)`(순수 검정과 혼합)로 했다가 "너무 어둡고 탁하다"는 피드백을 받아 어두운 슬레이트(`#334155`, 채도가 낮은 색)와 섞는 방식으로 바꿈 — 명도만 낮추는 것과 채도까지 같이 낮추는 것의 체감 차이가 커서, 색 어둡게 처리가 필요할 때는 검정이 아니라 저채도 중간톤과 섞는 걸 기본으로 고려할 것. `genderColorClass()`가 비동호인이면 `is-nonmember` 클래스를 붙이고, 자강과 동일한 패턴으로 관리자 쪽 9개 컴포넌트 CSS에 다크 변형을 미러링함(미리보기 2개는 위 항목대로 의도적으로 제외).
- **비동호인 표시 텍스트 = "입문"**: 내부 skill 값/키는 여전히 `'비동호인'` 그대로임(기존에 저장된 참가자 데이터·활성 세션이 깨지지 않게 하려고 데이터 자체는 안 바꿈) — 화면에 보이는 글자만 `src/data/skillLevels.js`의 `skillCode(skill)` 헬퍼를 거쳐 "입문"으로 바뀜. `SKILL_LABELS['비동호인']`도 `'입문'`으로 바꿔서 급수 선택 칩(SetupModal/SettingsModal/AddParticipantModal/ManageParticipantsModal/FilterBar)에 자동 반영되고, 그 외에 `player.skill`을 직접 찍던 곳(참가자 카드, 코트/대기열/수동매칭 슬롯, 추천매칭 카드, 게임로그, 대상선택 드롭다운, 엑셀 미리보기/안내문구 등 10여 곳)은 전부 `skillCode(player.skill)`로 감싸도록 고침 — `SKILL_LABELS[skill]`을 직접 쓰면 E/F가 배지에서 "E (초심)"처럼 길어지므로, 그 두 급수는 그대로 짧은 코드만 보여주고 비동호인만 바뀌게 이 헬퍼로 분리함. 엑셀 가져오기(`excelImport.js`)도 "입문"이 포함된 셀 값을 `NON_MEMBER_SKILL`로 인식하도록 별칭 추가. 참가자 카드의 급수 배지 폰트를 줄였던 것("비동호인" 4글자용)은 "입문"이 2글자(자강과 동일 길이)라 더 이상 필요 없어져서 제거함.
...
- **초반 동일 급수 우선(novelty 페널티)**: 같은 급수끼리 아직 안 해본 조합이 남아있을 때만 급수 교차(즐겜) 조합에 페널티 부여, 한 번이라도 겹치면 그 페널티는 빠르게 사라짐. **비동호인이 낀 조합은 이 페널티를 아예 안 받음** — 비동호인은 "같은 급수 상대"라는 개념 자체가 없는(같은 비동호인끼리 붙는 게 오히려 바람직하지 않은) 급수라서, A조 등 다른 급수의 신선한 동급수 페어가 남아있다는 이유로 "비동호인+A" 매칭이 페널티를 받으면 안 됨(비동호인은 오히려 상급자와 붙는 게 기본값). `sameSkillNoveltySupply`도 비동호인을 그룹화 대상에서 아예 제외함(비동호인-비동호인 미대전 쌍이 "신선한 동급수 공급"으로 잡혀서 다른 매칭까지 억제하는 걸 막기 위함).
...
- **남녀 기본 카드 색상**: `tokens.css`의 `--color-male`/`--color-female` 기본값은 사용자가 설정(`SettingsModal`)에서 직접 골라 쓰던 색(`#7597cc`/`#ff9999`)을 그대로 기본값으로 옮긴 것 — `-bg`/`-bg-dim` 파생값도 `App.jsx`가 커스텀 테마에 쓰는 것과 동일한 `lightenHex(0.85)`/`lightenHex(0.92)` 공식으로 재계산해서, "기본값"과 "커스텀 설정"이 항상 같은 방식으로 파생되게 맞춤. 설정 화면의 색상 피커 fallback(`theme.male ?? '...'`)도 같이 맞춰둠.
...
- **운동 시작/종료를 날짜까지 포함해 예약 가능**: 운동 시작(`SetupModal`)과 설정(`SettingsModal`) 모두 "시작 일시"·"종료 일시"를 `src/components/common/DateTimeStepInput.jsx`로 입력받음(`src/lib/time.js`의 `toDatetimeLocalValue`/`fromDatetimeLocalValue`로 타임스탬프↔"YYYY-MM-DDTHH:mm" 문자열 변환은 그대로 재사용). **`<input type="datetime-local">` + `step={300}`으로 먼저 구현했다가 두 가지 문제로 갈아엎음**: (1) `step`은 네이티브 피커의 스피너 증분에만 힌트를 줄 뿐이라 타이핑/스크롤로 31, 32분 같은 값이 그대로 들어감(→ onChange에서 반올림하는 방식으로 임시 패치했었음), (2) 그것과 별개로 **피커 자체의 분 스크롤 목록이 여전히 60개(1분 단위)를 다 보여줘서 5분 단위를 고르려면 스크롤을 한참 해야 하는 문제**는 반올림 패치로는 해결이 안 됨 — 결국 네이티브 피커를 통째로 버리고, 날짜(`<input type="date">`) + 시(`<select>` 00~23) + 분(`<select>`, 00/05/10.../55 12개만) 세 개짜리 커스텀 컴포넌트로 교체함. `session.startedAt`은 이제 "지금"이 아니라 **이 입력값 그대로**(미래 시각 예약 가능) — `initSession({..., startAt})`이 `Date.now()` 대신 이 값을 씀. 필드 순서는 두 모달 다 "시작 일시 → 종료 일시 → 참여 급수"(`SettingsModal`은 원래 "시작→참여급수→종료" 순이었는데 시작/종료가 한 쌍으로 붙어 있는 게 자연스럽다는 요청으로 바꿈). **시작 일시를 바꾸면 종료 일시는 항상 그 +3시간으로 리셋됨**(`SetupModal`/`SettingsModal` 둘 다 동일) — 예전엔 `SettingsModal`만 "종료 시각 고정, duration 역산" 방식이었는데, "시작을 다시 잡는다"는 건 대개 새로 계획하는 것이지 몇 분 미세조정이 아니라는 이유로 +3시간 기본값 리셋으로 바꿈(종료 일시 필드는 그 직후에 자유롭게 다시 조정 가능). 종료 일시만 바꿀 땐 시작은 그대로 두고 duration만 재계산(변경 없음). 기존 "운동 진행 시간" +/- 스테퍼는 종료 일시 입력과 기능이 겹쳐서 제거함.
- **운동 이름(선택)**: `session.name` — `SetupModal`(운동 생성 시)과 `SettingsModal` 둘 다 최상단에 텍스트 입력 필드로 노출, 비워두면 기본값. 설정돼 있으면 관리자 헤더(`Header.jsx`)와 참여자 화면(`DisplayView.jsx`)의 "🏸 배드민턴 게임 스케줄러" 타이틀 텍스트 부분이 그 이름으로 바뀜(이모지는 고정, `session.name || '배드민턴 게임 스케줄러'`) — Firebase로도 세션의 일부라 자동 동기화됨. `resetSession`(세션 초기화, 명단 유지)은 이름을 그대로 유지하고, `fullReset`/새 `initSession`은 초기화됨.
...
- **공지사항**: 헤더의 💬 버튼(`AnnouncementModal.jsx`, 설정 모달과 같은 `Modal` 패턴) → 텍스트 입력 후 "저장"하면 `announcement`(store) 갱신, "지우기"는 즉시 비움(모달은 안 닫힘, 계속 다시 쓸 수 있게). `main.jsx`의 Firebase push/pull 목록(session/players/courts/gamesById/queueOrder)에 `announcement`도 추가해서 관리자↔참여자 실시간 동기화됨. 참여자 화면(`DisplayView.jsx`)은 값이 있으면 **헤더(타이틀바)와 코트 그리드 사이**에 테두리만 있는 네모 박스로("📢" + 텍스트) 띄우고, 없으면(빈 문자열) 아예 안 보임 — 세션 시작 전 빈 화면에도 박스는 뜸(그땐 중앙에 표시). **처음엔 화면 맨 위(헤더보다도 위)에 주황 배경 배너로 만들었다가, "헤더랑 코트 사이에, 배경색 없이 네모칸만"으로 다시 요청받아 위치와 스타일을 바꿈** — 눈에 띄는 배너보다 튀지 않는 정보 박스를 원한 것. 관리자 헤더의 💬 버튼은 공지가 설정돼 있으면 파란 테두리로 표시되어, 모달을 안 열어도 지금 공지가 떠 있는지 알 수 있음. `fullReset`(전체 초기화)에서도 같이 비워짐.
