...
export function ParticipantCard({ player, selected, dimmed, reserved, liveGame }) {
  ...
  // Counts the game currently in progress too, not just completed ones.
  const displayedGames = player.totalGames + (player.status === '게임중' ? 1 : 0)
  ...
  return (
    ...
    >
      ...
      <div className="participant-card-footer">
        <span className="participant-card-games">{displayedGames}게임</span>
        {player.status !== '대기중' && (
